      ...
      items: [
        {
          ...
          description: "File path and line where the N+1 originates in your code, not buried in vendor frames"
        },
        {
          icon: "hash",
          title: "Query hash",
          description: "Reference identical queries across different sessions and track them over time"
        },
        {
          icon: "zap",
          title: "Occurrence count",
          description: "See exactly how many times each query repeated—no guessing about severity"
        },
        {
          icon: "table",
          title: "Table name",
          description: "Know which table is being queried repeatedly to prioritize fixes"
        }
      ]
    },
    {
      ...
      items: [
        {
          label: "Seq Scan",
          status: "bad",
          description: "Full table scan. Reads every row. Slow on large tables.",
        ...
        },
        {
          ...
          status: "warning",
          description: "Planner guessed wrong on row counts. Strategy may be suboptimal.",
...
The "Actual Rows" vs "Plan Rows" mismatch is particularly telling. When the planner thinks a table has 100 rows but it actually has 100,000, its cost calculations are wrong. This often happens after bulk data imports or major table changes before ANALYZE has run.`
    },
    {
      ...
      description: "Reference any captured query by its 12-character hash from perf:query output",
      output: `[
  {
    "Plan": {
      "Node Type": "Seq Scan",
...
Note that indexes aren't free—they slow down writes and consume disk space. But for read-heavy tables, especially those with large row counts and selective WHERE clauses, the tradeoff is almost always worth it.`
    },
    {
      ...
      steps: [
        { number: "1", title: "Find slow queries", description: "Run perf:query --slow=100 to identify queries over 100ms" },
        { number: "2", title: "Get the query plan", description: "Run perf:explain --hash=... to see execution details" },
        { number: "3", title: "Identify scan type", description: "Seq Scan means investigate. Index Scan usually means good." },
        { number: "4", title: "Add appropriate index", description: "CREATE INDEX CONCURRENTLY to avoid locking the table" },
        { number: "5", title: "Verify improvement", description: "Re-run EXPLAIN and compare times. Document the change." }
      ]
    },
    {
      type: "text",
...
After creating an index, PostgreSQL won't immediately start using it. You may need to run \`ANALYZE table_name\` to update statistics so the planner knows the index exists and is selective.`
    },
    {
      ...
      content: "When analyzing performance issues for specific tenants, you need to look at their actual data distribution. The same query can have wildly different plans depending on tenant data volume and selectivity.",
  ...
  "llm-coding-agents-and-performance-workflows": [
    {
      ...
      description: "LLM coding agents can run captures, analyze results, and apply fixes. Performance optimization becomes part of your CI/CD pipeline instead of a quarterly chore.",
      visual: {
        type: "agent-loop",
    ...
    },
    {
      type: "text",
      layout: "narrow",
      content: `The promise of LLM coding agents isn't just that they can write code—it's that they can run tools, interpret output, and iterate. When you pair this capability with laraperf's structured JSON output, you get something powerful: an agent that can autonomously find and fix performance issues.
...
The other difference is precision. When a human sees "slow query on line 47," they open the file and read around that area to understand context. An LLM agent does the exact same thing—it reads the surrounding code, understands the model relationships, and applies the appropriate fix (eager loading for N+1, index suggestions for slow scans, etc).`
    },
    {
      ...
      events: [
        ...
        { time: "0:01", command: "php artisan test", output: "Running test suite... (2 min)" },
        { time: "1:00", command: "perf:query --n1=3 --slow=50", output: "Found 2 N+1 candidates, 1 slow query >50ms" },
        { time: "1:02", command: "perf:explain --hash=a1b2c3d4", output: "{ \"Plan\": { \"Node Type\": \"Seq Scan\"... } }" },
        { time: "1:05", note: "Agent analyzes: N+1 in DealResource can be fixed with ::with('contact'). Slow query needs trigram index on email." },
        { time: "1:30", command: "perf:watch --seconds=60 && perf:query", output: "✓ 0 N+1 candidates, 0 slow queries. 47 queries → 2 queries." }
      ]
    },
    {
      type: "text",
      ...
      items: [
        {
          ...
          content: "Block PRs that introduce N+1 queries or slow queries exceeding configured thresholds"
        },
        {
          icon: "timer",
          title: "Regression testing",
          content: "Compare query counts before/after. Fail build if queries increased more than 10%"
        },
        {
          icon: "shield",
          title: "Production safety",
          content: "Run captures on staging before deploy. Catch issues that only show with real data volume"
        },
        {
          icon: "robot",
          title: "Auto-fix workflow",
          content: "Agent comments on PR with suggested fixes including ready-to-apply code patches"
        }
      ]
    },
    {
...
You can enhance this with automatic agent intervention. On failure, the action could trigger an agent to analyze the output, apply fixes, and push a commit to the PR. This turns performance regression detection into automatic performance regression correction.`
    },
    {
      ...
      left: {
        ...
        description: "Every command outputs structured JSON to stdout. Status lines go to stderr. Safe to pipe into jq, grep, or agent analysis tools."
      },
      right: {
        features: [
  ...
  "capturing-production-performance-data": [
    {
      ...
      description: "Production data shows true performance. Real user behavior. Real data volumes. Real concurrency patterns that development environments can't replicate.",
      stats: [
      ...
      items: [
        {
          ...
          description: "Queries append to JSON files using atomic operations. No database locks. No transaction overhead. The overhead of logging a query is less than 1% of the query execution time itself."
        },
        {
          icon: "detach",
          title: "Detached workers",
          description: "Capture runs in a separate process that doesn't block your web workers. If the capture process dies, your app keeps running. If your app gets busy, the capture keeps logging."
        },
        {
          icon: "memory",
          title: "Minimal memory footprint",
          description: "Queries stream to disk immediately. There's no in-memory buffer that could cause memory issues. Even a 10,000 query capture session uses less than 50MB of RAM."
        },
        {
          icon: "clock",
          title: "Time-boxed by default",
          description: "Captures auto-stop after the configured duration (default 5 minutes). Even if you forget to stop it, it won't run forever consuming disk space."
        }
      ]
    },
    {
...
If disk space runs low, the capture degrades gracefully—it might miss some queries, but it won't crash your app. If the JSON file grows large, rotation happens automatically. These failure modes are designed to be safe by default.`
    },
    {
      ...
      description: "Detach and capture for 5 minutes with a descriptive tag for later reference",
      ...
      items: [
        {
          ...
          description: "Buffer hit ratios under real load often differ dramatically from development. A query that uses the cache locally might be a cache miss in production due to memory pressure."
        },
        {
          title: "Concurrent query patterns",
          description: "Queries that conflict when run simultaneously. Lock contention, deadlocks, and serialization issues only show up with real concurrency."
        },
        {
          title: "Data volume impact",
          description: "Queries fast with 1K rows can be glacial with 1M rows. The PostgreSQL query planner changes strategies based on table statistics you can't replicate locally."
        },
        {
          title: "Missing indexes",
          description: "Sequential scans that weren't obvious in development because small tables fit in memory. Production I/O constraints make these painfully obvious."
        }
      ]
    },
    {
...
This targeted approach is more valuable than continuous monitoring because it captures the specific conditions causing problems. You see the query plans that execute slowly, the N+1 patterns that emerge under load, and the exact line numbers in your code where optimizations will have the most impact.`
    },
    {
      ...
      description: "Override the database at runtime without config changes or environment variable juggling",
      command: "php artisan perf:explain --hash=abc123 --db=tenant_acme_prod",
...
This is particularly useful for support scenarios. A tenant reports slowness on a specific feature. You can capture their specific database, analyze their specific query patterns, and give them specific optimization advice rather than generic best practices.`
    },
    {
      ...
      description: "Session files accumulate in storage/perf/. Clean up old captures to reclaim disk space.",
