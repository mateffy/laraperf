...
function smoothDampVec2(current, target, currentVelocity, smoothTime, maxSpeed, deltaTime) {
  ...
  out.copy(targetAdjusted.clone().add(change.clone().add(temp).multiplyScalar(exp)))
...
function smoothDampFloat(current, target, velRef, smoothTime, maxSpeed, deltaTime) {
  ...
  let out = targetAdj + (change + temp) * exp
...
export default function GridScan({
  ...
  useEffect(() => {
    ...
    const uniforms = {
      ...
      uScanDelay: { value: Math.max(0.0, scanDelay) },
    ...
    const tick = () => {
      ...
      const tiltSm = smoothDampFloat(tiltCurrent.current, tiltTarget.current, tiltVel.current, smoothTime, Infinity, dt)
      tiltCurrent.current = tiltSm.value
      tiltVel.current.v = tiltSm.v

      const yawSm = smoothDampFloat(yawCurrent.current, yawTarget.current, yawVel.current, smoothTime, Infinity, dt)
  ...
  }, [sensitivity, lineThickness, linesColor, scanColor, scanOpacity, gridScale, lineStyle, lineJitter, scanDirection, noiseIntensity, bloomIntensity, scanGlow, scanSoftness, scanPhaseTaper, scanDuration, scanDelay, skewScale, yBoost, tiltScale, yawScale, smoothTime])

  return (
    <div ref={containerRef} className={`gridscan-container${className ? ` ${className}` : ''}`} style={style}>
      {children}
    </div>
  )
}