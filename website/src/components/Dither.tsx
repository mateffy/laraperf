...
function DitheredWaves({
    ...
    }, [size, gl, uniforms.resolution]);

    // Update other props when they change
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        if (materialRef.current)
            materialRef.current.uniforms.waveColor.value.set(...waveColor);
    }, [waveColor, uniforms.waveColor]);

    useEffect(() => {
        uniforms.enableMouseInteraction.value = enableMouseInteraction ? 1 : 0;
        if (materialRef.current)
            materialRef.current.uniforms.enableMouseInteraction.value =
                enableMouseInteraction ? 1 : 0;
    }, [enableMouseInteraction, uniforms.enableMouseInteraction]);

    useEffect(() => {
        uniforms.mouseRadius.value = mouseRadius;
        if (materialRef.current)
            materialRef.current.uniforms.mouseRadius.value = mouseRadius;
    }, [mouseRadius, uniforms.mouseRadius]);

    // Animation loop - directly update material uniforms
    useFrame(({ clock }) => {
        if (!disableAnimation && materialRef.current) {
            materialRef.current.uniforms.time.value = clock.getElapsedTime();
        }
        if (enableMouseInteraction && materialRef.current) {
            materialRef.current.uniforms.mousePos.value.copy(mouseRef.current);
        }
    });
